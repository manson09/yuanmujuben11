...
1. 【强制金手指规则】
   ...
   ✅ 【新增情绪绑定规则】金手指出现时机必须绑定主角的核心情绪触发点（比如重生刚醒看见仇人/挚爱、被欺辱到临界点时），禁止凭空弹出系统弹窗；主角对应微反应必须是情绪的自然外化，不得使用无意义的模板动作。
2. 【爽感优先魔改规则】
...
2. 4-30秒：完成3个核心动作，缺一不可：
   ...
   ③ 【强制情绪锚点（原规则升级）】主角必须有仅观众可见的微反应：必须先出现1个0.1-0.2秒的碎片化创伤/执念闪回（仅观众可见，比如前世挚爱惨死的画面、被背叛的记忆碎片）→ 对应情绪微动作（攥拳/喉结滚动/瞳孔震颤/指节泛白）→ 最多1句短内心OS，OS必须绑定具体记忆锚点（比如提「十七天」「血泊」等具体细节，禁止空喊「我要报仇」「我要保护她」），总时长不超过3秒，不能被其他角色察觉
...
### 🚫 强制防踩坑补充规则（100%必须遵守）
1. 【隐忍逻辑强制规则】
   若主角存在「装弱/隐忍/隐身份」情节，必须在第1集前30秒和金手指一起向观众透底明确的隐忍理由（比如涉密任务倒计时、怕暴露身份连累女主家族、等待收网时机），禁止无理由隐忍；隐忍期间主角可暗中布局，不得出现「持有实锤却全程放任反派欺辱家人」的逻辑bug；【新增】隐忍仅针对剧内角色，面对核心仇人/保护对象时必须有仅观众可见的情绪微反应，禁止全程无情绪像执行任务的机器人。
2. 【冲突梯度强制规则】
...
4. 【人物逻辑强制规则】
   ...
   - 【新增主角情绪规则】若为重生/穿越/有执念的主角设定，情绪反应必须符合身份逻辑，面对核心仇人/保护对象时必须有仅观众可见的情绪细节（比如听见反派提女主名字时指节泛白），避免观众无法共情。
5. 【转场铺垫强制规则】
...
- 铺垫要求：先铺垫他人的轻视/嘲讽→主角可故意示弱放大反差，【新增】示弱同时可加入仅观众可见的情绪微反应（比如听见反派辱及家人时的眼底冷光）→打脸瞬间干脆利落，跟上全场震惊反应
...
// ---------------------- GeminiService 类（已修复所有自检bug） ----------------------
export class GeminiService {
  // 【2026修复】重构自检逻辑，支持按集数动态生成校验规则，彻底解决跨集误报问题
  private async ruleCheck(
    ...
    if (checkType === 'skeleton') {
      ...
      6. 反派是否有明确的作恶动机？
      7. 金手指是否明确了主角的微反应要求（包含闪回+情绪动作+锚点OS）？
      8. 是否符合对应子流派的专属防坑规则？
    ...
    // 大纲校验规则（不变）
      ...
      4. 转场的新矛盾是否有提前埋下的伏笔？
      5. 每集结尾是否有钩子？
      6. 第1集是否安排了主角对金手指的「闪回+情绪微动作+锚点OS」完整情绪链路情节？
      7. 是否符合对应子流派的专属防坑规则？
      【待检查内容】：${content.slice(0, 8000)}`;
    }

    // 【2026修复】脚本校验规则：按当前集数动态生成，彻底解决第5/6集还检查第1集内容的bug
      ...
      const firstEpisodeRules = isFirstEpisode ? [
        ...
        "第1集前30秒是否完成了「记忆闪回→情绪微动作→锚点化OS」的完整情绪链路？",
  ...
  async analyzeNovel(novelContent: string): Promise<string> {
    ...
    输出一个JSON对象，结构如下：
    {
        "base_info": {
            ...
            "protagonist": "主角姓名+身份+核心性格+隐藏底牌+【明确的隐忍动因（若有装弱/隐身份情节）】+【金手指出现时的完整情绪链路设定（闪回内容+微动作+锚点OS）】，必须包含强制加的穿越/系统金手指",
            "gold_finger": "强制加的金手指类型+能力+触发条件+【本阶段给出的信息/道具的明确使用节点】+【主角完整情绪链路要求（仅观众可见）】，明确使用边界（仅第1集前30秒出现1次）",
  ...
  async generateOutline(novelContent: string, analysisReport: string, targetStageNum: number = 1): Promise<string> {
    ...
    输出一个JSON对象，结构如下：
    {
        ...
        "episode_outlines": [
            {
                "episode_num": 1,
                "core_plot": "30字以内概括本集核心剧情，严格对应当前阶段的链路节点任务，必须包含金手指出现+主角完整情绪链路的情节",
    ...
    10. 第1集必须明确安排金手指出现+主角完整情绪链路的情节，仅观众可见
    `;
  ...
  // 第三阶段：生成全部脚本（【2026修复】正确传集数参数，修复跨集校验bug）
  async generateScripts(outlineText: string, phase: number, novelContent: string, formattingRef?: string): Promise<string> {
    ...
    for (const episode of outline.episode_outlines) {
      ...
      1. 【仅第1集生效】金手指仅能以字幕/系统提示的形式在第1集前30秒出现1次，禁止通过手机、台词、回忆、道具展示等任何形式二次暴露主角底牌！
      2. 【所有集生效】所有冲突场景必须明确写出3类旁观者的具体反应：① 踩主角的人说什么/做什么 ② 同情主角的人说什么/做什么 ③ 中立看热闹的人说什么/做什么，缺一类直接重写！
      3. 【仅第1集生效】第1集前30秒仅透底核心底牌+隐忍理由，不得透露多余的后续资源/计划，避免悬念削弱！
      4. 【所有集生效】禁止出现无情绪支撑的模板化主角动作/台词，重生/穿越类主角首次面对核心仇人/保护对象时，必须插入仅观众可见的0.1-0.2秒碎片化记忆闪回，内心OS必须绑定具体的创伤/执念细节，不得空泛喊口号！
      ...
      8. 第1集金手指出现时，是否匹配了「记忆闪回→情绪微动作→锚点化OS」的完整情绪链路？
    ...
    `;
      ...
      // 【2026修复】自检时正确传入当前集数和总集数，不会再出现跨集校验bug
      if (ENABLE_RULE_CHECK) {
