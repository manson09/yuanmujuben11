...
const MAX_RETRY = 3;
// 【2026优化新增】是否开启生成后规则自检，开启后会自动校验输出是否符合规则，不合格自动重试
...
| 都市赘婿 | 全剧终极钩子必须绑定「身份揭晓/复仇」，比如「隐藏3年的首富继承人，能不能在妻子被赶出家门时反杀所有看不起他的人？」；每10集阶段钩子绑定「妻女受辱→反杀」 | 【2026优化】身份底牌仅可在打脸最高潮揭晓，之前必须把嘲讽拉满；必须在第1集前30秒向观众透底男主隐忍的明确理由，女主/支持主角的长辈必须有至少2次暗中维护的细节，反派作恶动机必须围绕「谋夺家族财产/抢女主」展开 |
| 战神归来 | 全剧终极钩子必须绑定「护妻护女/复仇」，比如「消失5年的战神，能不能在女儿被拐卖前反杀所有仇家？」；每10集阶段钩子绑定「家人受辱→反杀」 | 【2026优化】战力/身份仅可提前给观众透底1次，禁止直接喊手下上场，必须先装弱再打脸；必须在第1集前30秒向观众透底男主隐姓埋名的明确理由（比如执行任务/报恩/保护家人），战力展示必须先解决核心危机再亮身份，禁止无意义的手下代打 |
...
| 年代创业 | 全剧终极钩子必须绑定「发家致富/弥补遗憾」，比如「重生回1998年的穷小子，能不能在千禧年之前赚够100万弥补前世妻女惨死的遗憾？」；每10集阶段钩子绑定「极品亲戚闹事/创业危机→反杀」 | 时代信息差仅可提前给观众透底1次，爽点突出「别人嫌赔本的生意主角赚翻」的反差 |
| 玄学爽文 | 全剧终极钩子必须绑定「化解劫数/积累功德」，比如「天生阴阳眼的小道士，能不能在18岁天劫之前攒够1000功德活下来？」；每10集阶段钩子绑定「事主不信→打脸改运」 | 玄学金手指仅可提前给观众透底1次，爽点突出「事主之前嚣张之后跪地拜谢」的反差 |
| 直播爽文 | 全剧终极钩子必须绑定「成为顶流/打脸黑料」，比如「被网暴退圈的明星，能不能靠直播算命半年内成为全网顶流反杀黑粉？」；每10集阶段钩子绑定「直播间被冲→打脸封神」 | 金手指仅可提前给观众透底1次，每集必须出现弹幕反应烘托爽感 |
| 规则怪谈 | 全剧终极钩子必须绑定「活过所有副本/破解规则真相」，比如「被拉进规则怪谈世界的打工人，能不能通关所有副本回到现实？」；每10集阶段钩子绑定「触发死亡规则→极限破解」 | 规则漏洞仅可提前给观众透底1次，爽点突出「只有观众知道主角卡了bug」的信息差 |
`;
...
- 【2026优化新增】打脸闭环要求：所有前期主动欺辱主角的核心反派、跳得最高的踩人旁观者，必须对应明确的惩罚/打脸情节（比如当众道歉、利益受损、社死、被追责），不得出现「反派随便溜走无惩罚」的情况；主角打脸时必须有明确的态度/台词回击，不得哑巴式打脸。
- 【2026优化新增】金手指使用要求：金手指提供的所有证据/信息/道具，必须在本阶段爽点爆发时全部使用，不得闲置，禁止「拿了实锤憋N集」的bug。
### 2. 荣获至宝类（核心：价值感）
    ...
    try {
      const response = await fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.3, // 【2026优化】从0.6调低，降低发散性，提升规则遵守度
          max_tokens: 16000, // 【2026优化】从12000调高，避免长脚本被截断
        }),
      });
...
export class GeminiService {
  ...
  private async ruleCheck(content: string, checkType: 'skeleton' | 'outline' | 'script'): Promise<{ pass: boolean, error?: string }> {
    const checkRules = {
      ...
      3. 爽点爆发时反派是否有对应惩罚，形成闭环？
      4. 所有伏笔是否回收，无废弃设定？
      5. 金手指是否仅在第1集前30秒出现1次，未抢戏？
      6. 冲突场景是否有3类旁观者的反应？`
    };
  ...
  async analyzeNovel(novelContent: string): Promise<string> {
    ...
    输出一个JSON对象，结构如下：
    {
        "base_info": {
            ...
            "protagonist": "主角姓名+身份+核心性格+隐藏底牌+【明确的隐忍动因（若有装弱/隐身份情节）】，必须包含强制加的穿越/系统金手指",
            "gold_finger": "强制加的金手指类型+能力+触发条件+【本阶段给出的信息/道具的明确使用节点】，明确使用边界（仅第1集前30秒出现1次）",
